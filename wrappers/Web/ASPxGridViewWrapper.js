...
dx.grid = function (controlID) {
    ...
    */
    var getColumnHeaders = function ($custWin) {
        return ($custWin || $el).find("td").filter(function () {
    ...
    */
    var getColumnHeader = function (fieldCaption, $custWin) {
        return ($custWin || $el).find("td").filter(function () {
            return isColumnHeaderID(this.id, $custWin && $custWin.length > 0) && $(this).find("td:contains(" + fieldCaption + ")").length > 0;
        });
    };
    ...
    */
    var getClearFilterButton = function () {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['ClearFilter'\\]*");
        return $el.find("a,img,input").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        }).first();
    };
    /**
    ...
    */
    var getApplyFilterButton = function () {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['ApplyMultiColumnAutoFilter'\\]*");
        return $el.find("a,img,input").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
    };
    ...
    */
    var getFilterApplyButton = function () {
        return $el.find("a[onclick*='ApplyMultiColumnAutoFilter']");
    };
    /**
    ...
    */
    var getNewRowButton = function (rowIndex) {
        ...
        if (row.length > 0)
            return getNewRowButtons().filter(function (c) { return row.has(c).length > 0; });
        return $();
    };
    /**
    ...
    */
    var getNewRowButtonFromHeader = function () {
        ...
        return getNewRowButtons().filter(function (b) {
            var $b = $(b);
            return headers.filter(function (i, h) { return $b.parents("#" + h.id).length > 0; }).length > 0;
        });
    };
    /**
    ...
    */
    var getDeleteButton = function (rowIndex) {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['Delete'," + rowIndex + "(,|\\])+");
        var buttons = $el.find("a, img").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
        if (buttons.length == 0) { 
            regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['Delete',-?\\d+(,|\\])+");
            var dataRow = findDataRow(rowIndex);
            buttons = dataRow.find("a, img").filter(function () {
                return this.onclick && regexp.test(this.onclick.toString());
            });
        }
        return $.makeArray(buttons);
    ...
    */
    var getEditButton = function (rowIndex) {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['StartEdit'," + rowIndex + "\\]*");
        var button = $el.find("a, img").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
        if (button.length == 0) {
            regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['StartEdit',-?\\d+(,|\\])+");
            var dataRow = findDataRow(rowIndex);
            button = dataRow.find("a, img").filter(function () {
                return this.onclick && regexp.test(this.onclick.toString());
            });
        }
        return button;
    ...
    */
    var getSelectButton = function (rowIndex) {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['Select'," + rowIndex + "\\]*");
        var button = $el.find("a, img").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
        if (button.length == 0) {
            regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['Select',-?\\d+(,|\\])+");
            var dataRow = findDataRow(rowIndex);
            button = dataRow.find("a, img").filter(function () {
                return this.onclick && regexp.test(this.onclick.toString());
            });
    ...
    */
    var getEditFormUpdateButton = function () {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['UpdateEdit'\\]*");
        return $el.find("a, img").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
    };
    /**
    ...
    */
    var getEditFormCancelButton = function () {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['CancelEdit'\\]*");
        return $el.find("a, img").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
    };
    ...
    var getCustomButton = function (buttonID, rowIndex) {
        var regexp = new RegExp("aspxGVScheduleCommand\\('" + clientID + "',\\['CustomButton'\\,'" + buttonID + "']*");
        return $("#" + clientID + DATA_ROW_AFFIX + rowIndex).find("img, a").filter(function () {
            return this.onclick && regexp.test(this.onclick.toString());
        });
    };
    /**
    ...
    */
    ...
    return {
        ...
        getFilterApplyButton: getFilterApplyButton,        
